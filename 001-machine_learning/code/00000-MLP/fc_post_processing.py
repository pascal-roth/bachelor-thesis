...
def load_checkpoint(nbr_net):
    ...
    path_pth = path.parents[2] / 'data/00000-MLP/{}_checkpoint.pth'.format(nbr_net)
...
def calc_acc(model, x_test, y_test, x_scaler, y_scaler, labels):
    ...
    enthalpy_values = x_test.drop_duplicates(['H'])
    enthalpy_values = enthalpy_values['H']

    abs_range = 0.005
    rel_range = 0.03

    acc_abs = np.zeros((len(enthalpy_values), len(labels)))  # create acc array
    acc_rel = np.zeros((len(enthalpy_values), len(labels)))  # create acc array
    acc_abs_rel = np.zeros((len(enthalpy_values), len(labels)))  # create acc array

    n = 0  # tracking parameter of test_loader length

    for i in range(len(enthalpy_values)):
        x_test_run = x_test[x_test.H == enthalpy_values.iloc[i]]
        x_test_run_indexes = x_test_run.index
        y_test_run = y_test.iloc[x_test_run_indexes, :]

        y_test_run = y_test_run.values

        y_test_run_nn = NN_output(model, x_test_run, x_scaler, y_scaler)

        correct_abs = np.zeros((len(y_test_run), len(labels)))
        correct_rel = np.zeros((len(y_test_run), len(labels)))
        correct_abs_rel = np.zeros((len(y_test_run), len(labels)))

        # check if samples are in 5% range
        for ii in range(len(labels)):
            target_run = np.squeeze(y_test_run[:, ii])
            output_run = np.squeeze(y_test_run_nn[:, ii])

            abs_value = np.amax(target_run) * abs_range

            for iii in range(len(y_test_run)):
                # check absolute accuracy
                if (target_run[iii] - abs_value) < output_run[iii] < (target_run[iii] + abs_value):
                    correct_abs[iii, ii] = 1
                else:
                    correct_abs[iii, ii] = 0

                # check relative accuracy
                if target_run[iii] * (1-rel_range) < output_run[iii] < target_run[iii] * (1+rel_range):
                    correct_rel[iii, ii] = 1
                else:
                    correct_rel[iii, ii] = 0

                # check absolute accuracy
                if (target_run[iii] - abs_value) < output_run[iii] < (target_run[iii] + abs_value) and \
                        target_run[iii] * (1-rel_range) < output_run[iii] < target_run[iii] * (1+rel_range):
                    correct_abs_rel[iii, ii] = 1
                else:
                    correct_abs_rel[iii, ii] = 0

            acc_abs[n, ii] = np.sum(correct_abs[:, ii]) / len(y_test_run)
            acc_rel[n, ii] = np.sum(correct_rel[:, ii]) / len(y_test_run)
            acc_abs_rel[n, ii] = np.sum(correct_abs_rel[:, ii]) / len(y_test_run)

        n += 1

    # calculate mean accuracy
    for j in range(len(labels)):
        acc_mean_abs = np.mean(acc_abs[:, j])
        acc_mean_rel = np.mean(acc_rel[:, j])
        acc_mean_abs_rel = np.mean(acc_abs_rel[:, j])
        print('For {} the absolute accuracy is: {:6.5e} \t relative accuracy is: {:6.5e} \t overall accuracy is: '
              '{:6.5e}'.format(labels[j], acc_mean_abs, acc_mean_rel, acc_mean_abs_rel))
...
def plot_outputs(output, y_test, samples_feature_run, features, labels, x_scaler, args):
    ...
    for i in range(len(labels)):
        ...
        if len(features) == 5:
            ...
            path = Path(__file__).resolve()
            path_plt = path.parents[2] / 'data/00000-MLP/{}_plt_{}_comp_PODE{}_{:.2f}_{:.0f}_{:.0f}.pdf'.\
            ...
            path = Path(__file__).resolve()
            path_plt = path.parents[2] / 'data/00000-MLP/{}_plt_{}_comp_PODE{}_{:.2f}_{:.0f}.pdf'.format \
...
def plot_IDT(IDT_MLP_PV, IDT_HR_PV, args):
    ...
    ax.plot(1000 / IDT_MLP_PV[:, 0], IDT_MLP_PV[:, 1], 'r-', label='MLP')
    ax.plot(1000 / IDT_HR_PV[:, 0], IDT_HR_PV[:, 1], 'b-', label='HR')
    ...
    ax.set_yticks([0.08, 0.085, 0.09, 0.095, 0.10])
    ax.set_yticklabels([0.08, 0.085, 0.09, 0.095, 0.10])

    plt.tight_layout()

    path = Path(__file__).resolve()
    path = path.parents[2] / 'data/00000-MLP/{}_plt_ID_pode{}_phi{}_p{}.pdf' \
