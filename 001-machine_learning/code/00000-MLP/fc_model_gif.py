...
def train(model, train_loader, valid_loader, criterion, optimizer, epochs, nbr_net, plot, inf_print, device, labels,
          x_scaler, y_scaler, number_train_run):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0 or epoch < 10:
            ...
            save_model(model, n_input, n_output, optimizer, criterion, epoch, features, labels, x_scaler,
    ...
    path_dir = path.parents[2] / 'data/00000-MLP'
    path_loss = path.parents[2] / 'data/00000-MLP/gif_{}_losses.csv'.format(nbr_net)
...
def save_model(model, n_input, n_output, optimizer, criterion, epoch, features, labels, x_scaler,
               y_scaler, number_train_run, inf_print, device):
    ...
    path = Path(__file__).resolve()
    path_dir = path.parents[2] / 'data/00000-MLP'
    path_mlp = path.parents[2] / 'data/00000-MLP/gif_checkpoint_{}.pth'.format(epoch)
