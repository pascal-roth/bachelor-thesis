...
parser.add_argument("--post", type=str, choices=['loss', 'output', 'plt_train', 'IDT'], default='IDT',
...
parser.add_argument("-nbr_run", "--number_test_run", type=str, default='004',
...
if args.post == 'loss':
    ...
    for i, temperature_run in enumerate(np.arange(x_samples_drop_min[0], x_samples_drop_max[0] + x_samples_drop_step,
        ...
        IDT_HR_PV[i, :] = (temperature_run, x_train_normalized['PV'].iloc[IDT_HR_location])

        # IDT of MLP ##############################################################################################
        y_samples_nn = fc_post_processing.NN_output(model, x_train, x_scaler, y_scaler)
        IDT_MLP_location = np.argmax(y_samples_nn)
        IDT_MLP_PV[i, :] = (temperature_run, x_train_normalized['PV'].iloc[IDT_MLP_location])
