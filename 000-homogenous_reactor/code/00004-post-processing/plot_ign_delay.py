...
mechanism_all = np.array([['he_2018.xml', 'DMM3'], ['cai_ome14_2019.xml', 'OME3'], ['sun_2017.xml', 'DMM3'],
                          ['cai_ome14_2019.xml', 'OME4']])


def plot_delays(pode, equivalence_ratio, reactorPressure):
    fig = plt.figure()
    ...
    if pode <= 3:
        ...
        he = np.flip(loaddata_delays(mechanism_all[0, 0], equivalence_ratio, reactorPressure, pode))
        sun = np.flip(loaddata_delays(mechanism_all[1, 0], equivalence_ratio, reactorPressure, pode))
        cai = np.flip(loaddata_delays(mechanism_all[2, 0], equivalence_ratio, equivalence_ratio, pode))
    ...
    path = Path(__file__).parents[2] / 'data/00004-post-processing/delays_{}_{}_PODE{}.pdf'.format(equivalence_ratio,
                                                                                                   reactorPressure,
                                                                                                   pode)
    plt.savefig(path)
