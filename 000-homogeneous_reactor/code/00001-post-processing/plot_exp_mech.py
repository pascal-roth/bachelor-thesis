...
def loaddata_sim(mechanism, nbr_run, equivalence_ratio, reactorPressure, pode):
    ...
    path = path.parents[2] / 'data/00000-reactor-OME/{}/{}_exp_delays.csv'.format(mechanism[0], nbr_run)
...
def loaddata_exp(OME, reactorPressure, equivalence_ratio):
    path = Path(__file__).resolve()
    path = path.parents[2] / 'data/00001-post-processing/data_exp/Exp_{}_{}_{}.csv'.format(OME, equivalence_ratio, reactorPressure)
...
path = Path(__file__).resolve()
path = path.parents[2] / 'data/00001-post-processing/delays_PODE{}_phi{}_p{}.pdf'\
